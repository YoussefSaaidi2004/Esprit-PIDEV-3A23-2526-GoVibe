...
public class AdminReclamationController implements Initializable {
    ...
    private void filterAndDisplay() {
        ...
        List<Reclamation> filtered = allReclamations.stream()
            .filter(r -> {
                ...
                // Normalize: remove accents and convert to uppercase for comparison
                String normalizedStatus = java.text.Normalizer.normalize(status, java.text.Normalizer.Form.NFD)
                    .replaceAll("\\p{InCombiningDiacriticalMarks}+", "")
                    .replace(" ", "_")
                    .toUpperCase();
                return normalizedStatus.equals(r.getStatus());
            })
            .filter(r -> {
    ...
    private void showEditDialog(Reclamation r) {
        ...
        dialog.showAndWait().ifPresent(result -> {
            ...
            try {
                // If there's a response, add it first (repondre sets status to RESOLU)
                if (parts.length == 2 && !parts[1].isEmpty()) {
                    service.repondre(r.getId(), parts[1]);
                }
                // Then update status to the selected one (this overrides RESOLU if different)
