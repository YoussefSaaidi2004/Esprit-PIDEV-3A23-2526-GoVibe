...
public class LocationListController {
    ...
    @FXML
    private ListView<Location> locationList; // Kept for compatibility but unused
    @FXML
    ...
    private final ObservableList<Location> pageItems = FXCollections.observableArrayList(); // Keep for compatibility
    ...
    @FXML
    private javafx.scene.layout.TilePane locationsGrid;
    ...
    private void updatePageItems() {
        locationsGrid.getChildren().clear();
        ...
        List<Location> itemsToShow = currentFiltered.subList(fromIndex, toIndex);

        for (Location item : itemsToShow) {
            locationsGrid.getChildren().add(createLocationCard(item));
        }
    }

    private VBox createLocationCard(Location item) {
        VBox card = new VBox();
        card.setSpacing(10);
        card.setPadding(new javafx.geometry.Insets(15));
        card.setStyle("-fx-background-color: white; -fx-background-radius: 12; " +
                      "-fx-effect: dropshadow(gaussian, rgba(0,0,0,0.05), 8, 0, 0, 2); " +
                      "-fx-pref-width: 300; -fx-pref-height: 320;");
        ...
        if (imageUrl != null && !imageUrl.isEmpty() && (imageUrl.startsWith("http") || imageUrl.startsWith("file:"))) {
            try {
                imageView.setImage(new Image(imageUrl, 270, 150, true, true, true));
            } catch (Exception e) {
                // partial fail
            }
        }
        ...
        imgContainer.setStyle("-fx-background-color: #f8f8f8; -fx-background-radius: 8;");
        imgContainer.setPrefSize(270, 150);

        // Content
        Label refLabel = new Label("REF: " + safeTrim(item.getReference()));
        refLabel.setStyle("-fx-font-size: 11; -fx-text-fill: #888;");

        String voitureName = getVoitureLabel(item);
        Label titleLabel = new Label(voitureName);
        titleLabel.setStyle("-fx-font-size: 16; -fx-font-weight: bold; -fx-text-fill: #013220;");
        
        Label dateLabel = new Label("📅 " + item.getDateDebut() + " ➝ " + item.getDateFin());
        dateLabel.setStyle("-fx-font-size: 13; -fx-text-fill: #5a7d6e;");

        Label priceLabel = new Label(String.format("%.2f TND", item.getMontantTotal()));
        priceLabel.setStyle("-fx-font-size: 16; -fx-font-weight: bold; -fx-text-fill: #50C878;");

        Label statusLabel = new Label(item.getStatut() != null ? item.getStatut().toString() : "");
        String statusColor = "#888"; 
        if (item.getStatut() != null) {
             switch(item.getStatut().toString()) {
                 case "CONFIRMEE": statusColor = "#27AE60"; break;
                 case "EN_ATTENTE": statusColor = "#F39C12"; break;
                 case "ANNULEE": statusColor = "#E74C3C"; break;
                 case "TERMINEE": statusColor = "#34495E"; break;
             }
        }
        statusLabel.setStyle("-fx-background-color: " + statusColor + "20; -fx-text-fill: " + statusColor + "; " +
                             "-fx-font-weight: bold; -fx-padding: 4 8; -fx-background-radius: 4;");
        ...
        editBtn.setStyle("-fx-background-color: #F39C12; -fx-text-fill: white; -fx-background-radius: 20;");
        editBtn.setOnAction(e -> openEdit(item, editBtn));
        
        Button deleteBtn = new Button("🗑");
        deleteBtn.setStyle("-fx-background-color: #E74C3C; -fx-text-fill: white; -fx-background-radius: 20;");
        deleteBtn.setOnAction(e -> deleteFromRow(item));
        
        Button pdfBtn = new Button("📄");
        pdfBtn.setStyle("-fx-background-color: #3498DB; -fx-text-fill: white; -fx-background-radius: 20;");
        pdfBtn.setOnAction(e -> saveCopy(item.getContratPdf(), "Contrat", "PDF", "*.pdf"));
        
        Region spacer = new Region();
        HBox.setHgrow(spacer, Priority.ALWAYS);
        
        actions.getChildren().addAll(priceLabel, spacer, editBtn, deleteBtn, pdfBtn);

        card.getChildren().addAll(imgContainer, refLabel, titleLabel, dateLabel, statusBox, actions);
    ...
    private void handleBackToCars() {
        SceneNavigator.switchTo("/UserVoituresView.fxml", backToCarsButton);
    }
    ...
    private void saveCopy(String path, String title, String description, String... extensions) {
        ...
        File destination = chooser.showSaveDialog(locationsGrid.getScene().getWindow());
        if (destination == null) {
            return;
        }
        try {
    ...
    private void handleGoHotels() {
        org.example.mains.MainApp.switchScene("/chambre-view.fxml", "Hôtels");
    }
    ...
    private void handleLogout() {
        ...
        SceneNavigator.switchTo("/org/example/LoginView.fxml", locationsGrid);
    }
    ...
    private void handleMyReservations() {
        org.example.mains.MainApp.switchScene("/chambre-view.fxml", "Mes Réservations");
    }
