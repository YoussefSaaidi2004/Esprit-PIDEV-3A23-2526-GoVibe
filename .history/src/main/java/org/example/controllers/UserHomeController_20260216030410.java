...
public class UserHomeController {
    ...
    public void initialize() {
        if (heroImage != null && rootStack != null) {
            heroImage.fitWidthProperty().bind(rootStack.widthProperty());
        }
        
        // Setup scroll to top button visibility
        if (mainScrollPane != null && scrollToTopBtn != null) {
            mainScrollPane.vvalueProperty().addListener((obs, oldVal, newVal) -> {
                if (newVal.doubleValue() > 0.1) {
                    scrollToTopBtn.setVisible(true);
                } else {
    ...
    private void handleScrollToTop() {
        if (mainScrollPane != null) {
            // Smooth scroll would require animation, standard setVvalue is instant
