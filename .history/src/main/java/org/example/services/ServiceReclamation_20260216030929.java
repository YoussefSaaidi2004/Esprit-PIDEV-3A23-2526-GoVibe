...
import org.example.utils.UnifiedDatabaseManager;
...
public class ServiceReclamation {
    ...
    private final UnifiedDatabaseManager dbManager = UnifiedDatabaseManager.getInstance();
    ...
    public void supprimer(int id) throws SQLException {
        String sql = "DELETE FROM reclamation WHERE id=? AND status=?";

        try (Connection conn = dbManager.getConnection();
             PreparedStatement pst = conn.prepareStatement(sql)) {

            pst.setInt(1, id);
            pst.setString(2, Reclamation.STATUS_EN_ATTENTE);
