...
public class AdminLocationListController {
    ...
    private void handleGoFlights() {
        SceneNavigator.switchTo("/views/flight-management.fxml", root);
    }

    @FXML
    private void handleGoCheckouts() {
        SceneNavigator.switchTo("/views/checkout-management.fxml", root);
    }
    ...
    private void openHotelModule(String initialView) {
        try {
            javafx.fxml.FXMLLoader loader = new javafx.fxml.FXMLLoader(getClass().getResource("/main-layout.fxml"));
    ...
    private void handleGoActivites() {
        SceneNavigator.switchTo("/Dashboard.fxml", root);
    }

    @FXML
    private void handleGoForums() {
        SceneNavigator.switchTo("/poste-forumviews/ListForum.fxml", root);
    }
    ...
    private void updateStatusFromRow(Location item, StatutLocation newStatus) {
        if (item == null || newStatus == null) {
            return;
        }
        try {
            locationService.updateStatusAndHandleOverlap(item.getIdLocation(), newStatus);
            refreshList();
        } catch (RuntimeException ex) {
            showAlert("Erreur", ex.getMessage());
        }
    ...
    private class LocationCell extends ListCell<Location> {
        ...
        private final ComboBox<StatutLocation> statusBox = new ComboBox<>();
        private final Button applyStatusButton = new Button("Appliquer");
        ...
        LocationCell() {
            ...
            applyStatusButton.getStyleClass().addAll("row-btn", "row-btn-edit");
            ...
            statusBox.setItems(FXCollections.observableArrayList(StatutLocation.values()));

            left.getChildren().addAll(title, subtitle, meta);
                center.getChildren().addAll(statusBox, applyStatusButton,
                    viewContractButton, viewQrButton, saveContractButton, saveQrButton);
        ...
        protected void updateItem(Location item, boolean empty) {
            ...
            statusBox.setValue(item.getStatut() != null ? item.getStatut() : StatutLocation.EN_ATTENTE);
            ...
            applyStatusButton.setOnAction(event -> updateStatusFromRow(item, statusBox.getValue()));
